...
function fixARVideo() {
    document.querySelectorAll('#viewer-ar video').forEach(v=>{
...
function addSteamEffect(model) {
    ...
    function animateSteam(){
        steamAnimId=requestAnimationFrame(animateSteam); if(!steamParticles)return; frame++;
        const p=steamParticles.geometry.attributes.position;
        for(let i=0;i<count;i++){p.array[i*3+1]+=speeds[i];p.array[i*3]+=Math.sin(frame*0.02+offsets[i])*0.003;p.array[i*3+2]+=Math.cos(frame*0.02+offsets[i])*0.003;if(p.array[i*3+1]>3){p.array[i*3]=(Math.random()-0.5)*1.5;p.array[i*3+1]=0;p.array[i*3+2]=(Math.random()-0.5)*1.5;}}
        p.needsUpdate=true; mat.opacity=0.3+Math.sin(frame*0.05)*0.1;
    }
    animateSteam();
}
